...
void pipe_print_state(Pipeline *p){
   ...
   for(width_i = 0; width_i < PIPE_WIDTH; width_i++) {
       ...
       if(p->EX_latch[width_i].valid == true) { //IMPT
         for(int ii = 0; ii < MAX_WRITEBACKS; ii++) {
            if(p->EX_latch[ii].valid) // Check in exec buffer
...
void pipe_cycle_schedule(Pipeline *p) {
  ...
  if(SCHED_POLICY==0){
    ...
    for(int ii=0; ii<PIPE_WIDTH; ii++){
      ...
      // Check if the oldest instruction in the ROB is ready[sources are ready..] (and unscheduled)
      // If yes, schedule it
      // If no, stall
      ...
      for (int kk = 0; kk < NUM_ROB_ENTRIES; kk++) { 
...
void pipe_cycle_writeback(Pipeline *p){
  ...
  // YOU HAVE TO PARSE THE EXEC-LATCH SOMEHOW!
  // Ans: Search in 'exec' latch!
  for(int ii = 0; ii < MAX_WRITEBACKS; ii++) {
...
void pipe_cycle_commit(Pipeline *p) {
  ...
  // checks if 'head_ptr' is ready to commit
  if (ROB_check_head(p->pipe_ROB)) { 
    ...
    // we should be completing the whole ROB, so count till the point when ROB is empty
    // Condition for ROB to be empty: (head == tail && head->invalid instn)
    // OR
    // when 'p->pipe_ROB->ROB_Entries[p->pipe_ROB->head_ptr].inst.inst_num' >= p->halt_inst_num
    // ^ doesn't work.. keep looping on the 100th instruction!


    if( p->pipe_ROB->head_ptr == p->pipe_ROB->tail_ptr && 
       (p->pipe_ROB->ROB_Entries[p->pipe_ROB->tail_ptr].valid == false)) {
